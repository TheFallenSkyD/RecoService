...
class UserKnn:
    ...
    def get_matrix(
        ...
        self.watched = (
            df.groupby(user_col, as_index=False).agg({item_col: list}).rename(columns={user_col: "similar_user_id"})
        )
    ...
    def predict(self, test: pd.DataFrame, N_recs: int = 10):
        ...
        recs = recs[~(recs["user_id"] == recs["similar_user_id"])].merge(
            self.watched, on=["similar_user_id"], how="left"
        )
